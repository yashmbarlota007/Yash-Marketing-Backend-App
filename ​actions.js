...
function performSearch() {
    ...
    const matches = window.appData.rawArray.filter(o => (o.orderId && o.orderId.toLowerCase().includes(query)) || (o.shopName && o.shopName.toLowerCase().includes(query)) || (o.phone && o.phone.includes(query))).slice(0, 10);
    ...
    resultsDiv.innerHTML = matches.map(o => {
        ...
        return `<div onclick="closeSearch(); openModal('${o.orderId}')" class="p-3 border-b border-slate-800 hover:bg-slate-800 cursor-pointer flex justify-between items-center transition-colors rounded-lg mb-1"><div><div class="font-black text-white text-sm flex items-center">${o.orderId} ${badge}</div><div class="text-xs text-indigo-400 font-bold">${o.shopName}</div></div><div class="text-right"><div class="text-[10px] bg-indigo-900/50 border border-indigo-500/30 px-2 py-1 rounded text-indigo-300 font-black mb-1">Stage ${o.completedStages}/9</div><div class="text-[9px] text-emerald-400 font-black uppercase">${o.paymentMode}</div></div></div>`;
    }).join('');
}
...
async function saveHandover() {
    ...
    try {
        let res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'saveHandover', staffName: currentUserName, note: note }) });
        let data = await res.json();
        if(data.status === 'success') { document.getElementById('handoverNoteInput').value = ''; fetchHandoverNotes(); } else alert("Failed to save note.");
...
function renderMdoDashboard() {
    ...
    let shopStats = {}, skuStats = {}, staffScores = {}, cityStats = {}, brandStats = {}, stageStuck = Array(9).fill(0);
    const nowTime = new Date().getTime();

    filteredData.forEach(o => {
        totalOrderValue += (o.totalValue || 0);
        if (o.completedStages >= 9 || o.isFullyCompleted) totalCompleted++;
        else { 
            if (o.completedStages < 9) stageStuck[o.completedStages]++; 
        ...
        o.items.forEach(i => {
            let qty = parseInt(i.qty || 1), itemVal = parseFloat(i.totalValue || 0);
            totalVolume += qty; shopStats[o.shopName].items += qty;
            let cleanName = i.name ? i.name.trim() : "Unknown"; 
            if(!skuStats[cleanName]) skuStats[cleanName] = { qty: 0, value: 0 }; 
            skuStats[cleanName].qty += qty; skuStats[cleanName].value += itemVal;
        ...
        cityStats[city].orders += 1; cityStats[city].value += (o.totalValue || 0);
    });
    ...
    let mdoHtml = `
        <div class="flex justify-between items-center bg-[#0B1121] border border-slate-800 p-4 rounded-2xl mb-4 shadow-lg">
            <div><h2 class="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-500 tracking-widest uppercase">🔴 LIVE WAR ROOM</h2></div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            <div class="glass-panel p-4 rounded-2xl border-t-4 border-t-indigo-500 hover-card">
                <div class="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Total Volume & Value</div>
                <div class="flex items-end gap-2 mt-2"><span class="text-2xl font-black text-white">${totalVolume} U</span><span class="text-xs text-indigo-400 font-bold mb-1 pb-1">₹${totalOrderValue.toLocaleString()}</span></div>
            </div>
            <div class="glass-panel p-4 rounded-2xl border-t-4 border-t-emerald-500 hover-card">
                ...
                <span class="text-2xl font-black text-emerald-400 mt-2 block">${Math.round(completionRate)}%</span>
                <div class="text-[9px] text-slate-500 mt-1 font-semibold">${totalCompleted} out of ${filteredData.length} Delivered</div>
            </div>
            <div class="glass-panel p-4 rounded-2xl border-t-4 ${slaBreaches > 0 ? 'border-t-pink-500 bg-pink-500/5' : 'border-t-slate-700'} hover-card">
                <div class="text-[10px] ${slaBreaches > 0 ? 'text-pink-400' : 'text-slate-400'} uppercase font-black tracking-widest mb-1 flex items-center gap-2">${slaBreaches > 0 ? '<span class="animate-pulse">⚠️</span>' : '✅'} SLA Breaches</div>
                <span class="text-2xl font-black ${slaBreaches > 0 ? 'text-pink-500' : 'text-white'} mt-2 block">${slaBreaches}</span>
            ...
            <div class="glass-panel p-4 rounded-2xl border-t-4 border-t-orange-500 hover-card">
                ...
                <div class="text-sm font-black text-orange-400 truncate mt-2 leading-tight">${frictionIndex}</div>
                <div class="inline-block mt-2 bg-orange-500/10 text-orange-400 text-[10px] font-bold px-2 py-0.5 rounded">${maxStuck} Stuck Here</div>
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div class="glass-panel p-5 rounded-2xl lg:col-span-1">
                <h3 class="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2"><span class="text-yellow-400">🔥</span> Hot Movers</h3>
                <div class="space-y-3">
                    ${sortedSKUs.length > 0 ? sortedSKUs.map((sku, i) => `<div class="bg-[#0B1121] p-2.5 rounded-lg border border-slate-800"><div class="text-xs font-bold text-slate-300 truncate mb-1">${i+1}. ${sku[0]}</div><div class="flex justify-between items-end"><span class="text-sm font-black text-yellow-400">${sku[1].qty} <span class="text-[9px] text-slate-500">QTY</span></span><span class="text-xs font-black text-emerald-400">₹${sku[1].value.toLocaleString()}</span></div></div>`).join('') : `<p class="text-slate-500 text-sm italic">No SKU data.</p>`}
                </div>
            </div>
            <div class="glass-panel p-5 rounded-2xl lg:col-span-1">
                <h3 class="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2"><span class="text-amber-400">🏆</span> Leaderboard</h3>
                <div class="space-y-3">
                    ${sortedStaff.length > 0 ? sortedStaff.map((staff, i) => `<div class="flex justify-between items-center bg-[#0B1121] p-2.5 rounded-lg border border-slate-800"><span class="text-xs font-bold text-slate-300">#${i+1} ${staff[0]}</span><span class="text-xs font-black text-emerald-400">${staff[1] * 10} Pts</span></div>`).join('') : `<p class="text-slate-500 text-sm italic">No staff data.</p>`}
                </div>
            </div>
            <div class="glass-panel p-5 rounded-2xl lg:col-span-1">
                <h3 class="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2"><span class="text-emerald-400">👑</span> VIP Dealers</h3>
                <div class="space-y-3 max-h-60 overflow-y-auto hide-scrollbar pr-2">
                    ${sortedShops.length > 0 ? sortedShops.slice(0, 5).map((v, i) => `<div class="relative w-full bg-[#0B1121] rounded-lg p-2.5 overflow-hidden border border-slate-800 group"><div class="absolute top-0 left-0 h-full bg-gradient-to-r from-emerald-900/40 to-transparent" style="width: ${(v[1].value / topShopVolume) * 100}%"></div><div class="relative z-10"><div class="text-xs font-bold text-slate-200 truncate group-hover:text-white">${v[0]}</div><div class="text-[9px] text-slate-400 uppercase tracking-widest mb-1">📍 ${v[1].area}</div><div class="text-sm font-black text-emerald-400">₹${v[1].value.toLocaleString()}</div></div></div>`).join('') : `<p class="text-slate-500 text-sm italic">No dealer data.</p>`}
                </div>
            </div>
            <div class="glass-panel p-5 rounded-2xl lg:col-span-1">
                <h3 class="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2"><span class="text-blue-400">📍</span> Area Heatmap</h3>
                <div class="space-y-3">
                    ${sortedCities.length > 0 ? sortedCities.map((city, i) => `<div class="bg-[#0B1121] p-2.5 rounded-lg border border-slate-800"><div class="text-xs font-bold text-slate-300 truncate mb-1">#${i+1} ${city[0]}</div><div class="flex justify-between items-end"><span class="text-sm font-black text-blue-400">${city[1].orders} <span class="text-[9px] text-slate-500">ORD</span></span><span class="text-xs font-black text-emerald-400">₹${city[1].value.toLocaleString()}</span></div></div>`).join('') : `<p class="text-slate-500 text-sm italic">No area data.</p>`}
                </div>
            </div>
            <div class="glass-panel p-5 rounded-2xl lg:col-span-1 flex flex-col">
                <h3 class="text-xs font-black text-white uppercase tracking-widest mb-4 flex justify-between items-center"><span class="flex items-center gap-2"><span class="text-purple-400">📊</span> Brand Matrix</span></h3>
                <div class="flex-1 w-full h-40 relative flex justify-center items-center"><canvas id="brandPieChart"></canvas></div>
            </div>
        </div>
    `;
    ...
    setTimeout(() => { 
        ...
        if (ctx) {
            ...
            if (labels.length === 0) { labels.push("No Data"); dataVals.push(1); }
            brandChartInstance = new Chart(ctx.getContext('2d'), {
                type: 'doughnut',
                data: { labels: labels, datasets: [{ data: dataVals, backgroundColor: ['#ef4444', '#ec4899', '#f59e0b', '#3b82f6', '#10b981', '#a855f7', '#06b6d4', '#64748b'], borderWidth: 0, hoverOffset: 5 }] },
                options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: '#94a3b8', font: { size: 10 }, boxWidth: 10 } }, tooltip: { callbacks: { label: function(context) { let label = context.label || ''; let val = context.raw || 0; let percent = totalMatrixValue > 0 ? Math.round((val / totalMatrixValue) * 100) : 0; return `${label}: ₹${val.toLocaleString()} (${percent}%)`; } }, backgroundColor: 'rgba(15, 23, 42, 0.9)', titleFont: { size: 11 }, bodyFont: { size: 13, weight: 'bold' }, padding: 10, borderColor: '#6366F1', borderWidth: 1 } } }
            });
        }
    }, 100);
}

function setStageFilter(stageIndex) { activeStageFilter = stageIndex; renderPipeline(); }
...
async function executeUnblock(orderId) {
    ...
    try {
        let res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'unblockOrder', orderId: orderId }) });
        let data = await res.json();
        if (data.status === 'success') { showNotification("OVERRIDE SUCCESS", "Order Unlocked."); fetchOrders(true); } else alert("Failed to unblock order.");
    } catch(e) { alert("Network error."); }
}
...
function renderPipeline() {
    ...
    pendingOrders.forEach(o => { 
        if (o.completedStages < 9) stageCounts[o.completedStages]++; 
        if (o.completedStages < 3) shopPendingCount[o.shopName] = (shopPendingCount[o.shopName] || 0) + 1; 
    });

    let filterHtml = `<div onclick="setStageFilter(null)" class="snap-start cursor-pointer ${activeStageFilter === null ? 'bg-indigo-600 border-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.4)]' : 'bg-[#131C31] border-slate-800 hover:bg-slate-800'} flex flex-col justify-between p-3 rounded-xl border min-w-[120px] shrink-0 transition-all"><span class="${activeStageFilter === null ? 'text-indigo-200' : 'text-slate-500'} text-[9px] font-black uppercase tracking-widest mb-1">Overview</span><span class="text-white text-sm font-black mb-2">All Pending</span><div class="flex items-center justify-between mt-auto pt-2 border-t border-slate-700/50"><span class="text-[10px] ${activeStageFilter === null ? 'text-indigo-200' : 'text-slate-600'} font-bold">COUNT</span><span class="${activeStageFilter === null ? 'bg-white text-indigo-900' : 'bg-[#0B1121] text-slate-400'} px-2 py-0.5 rounded text-xs font-black">${pendingOrders.length}</span></div></div>`;
    
    STAGE_NAMES.forEach((name, idx) => {
        let isActive = activeStageFilter === idx, orderCount = stageCounts[idx], filterName = name.split(' (')[0]; 
        if (idx === 6) filterName = "Delivery Comms";
        filterHtml += `<div onclick="setStageFilter(${idx})" class="snap-start cursor-pointer ${isActive ? 'bg-indigo-600 border-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.4)]' : 'bg-[#131C31] border-slate-800 hover:bg-slate-800'} flex flex-col justify-between p-3 rounded-xl border min-w-[130px] shrink-0 transition-all"><span class="${isActive ? 'text-indigo-200' : 'text-slate-500'} text-[9px] font-black uppercase tracking-widest mb-1">Step ${idx+1}</span><span class="${isActive ? 'text-white' : 'text-slate-400'} text-xs font-bold mb-2 leading-tight">${filterName}</span><div class="flex items-center justify-between mt-auto pt-2 border-t border-slate-700/50"><span class="text-[10px] ${isActive ? 'text-indigo-200' : 'text-slate-600'} font-bold">STUCK</span><span class="${orderCount > 0 ? (isActive ? 'bg-white text-indigo-900' : 'bg-pink-500/20 text-pink-400 border border-pink-500/20') : (isActive ? 'bg-indigo-900 text-indigo-300' : 'bg-[#0B1121] text-slate-600')} px-2 py-0.5 rounded text-xs font-black">${orderCount}</span></div></div>`;
    });
    ...
    if (displayOrders.length === 0) { 
        grid.innerHTML = `<div class="col-span-full bg-[#131C31] border border-slate-800 p-10 rounded-2xl text-center"><div class="text-4xl mb-3">🍃</div><p class="text-slate-400 text-lg font-bold">Inbox Zero for this view.</p></div>`; 
        return; 
    }
    ...
    displayOrders.forEach(order => {
        ...
        if (order.completedStages < 9) {
            if (order.completedStages === 6) nextStepName = isCod ? "Call 2" : "WhatsApp 2";
            else nextStepName = STAGE_NAMES[order.completedStages].split(' (')[0];
        }
        
        let orderDateObj = parseCustomDate(order.date), exactDateStr = formatExactDate(order.date), timeData = getTimeAgoUI(order.date);
        let orderItemsCount = order.items.reduce((sum, item) => sum + parseInt(item.qty || 1), 0);
        
        let splitBadge = String(order.orderId).toUpperCase().includes('SPLIT') ? `<span class="bg-purple-900/30 text-[9px] px-2 py-1 rounded text-purple-400 font-bold border border-purple-500/30 shrink-0">✂️ SPLIT</span>` : "";
        let lockOverlay = "", clickAction = `onclick="openModal('${order.orderId}')"`, cursorStyle = "cursor-pointer", vipClass = order.isVIP ? "vip-corridor bg-[#131C31]" : "bg-[#131C31] hover:border-indigo-500 border-slate-800";
        ...
        if (order.creditLocked && !order.ceoOverride) {
            ...
            lockOverlay = `<div class="absolute inset-0 bg-black/80 backdrop-blur-[2px] z-20 flex flex-col items-center justify-center p-4 text-center rounded-2xl border-2 border-red-500"><span class="text-3xl mb-1">🛑</span><span class="text-white font-black text-xs tracking-widest uppercase">SYSTEM LOCK</span><span class="text-red-400 font-bold text-[10px] mt-1 uppercase tracking-wider">${order.lockReason}</span>${unlockBtn}</div>`;
            clickAction = ""; cursorStyle = "cursor-not-allowed"; vipClass = "bg-red-900/10 border-red-500/50";
        }
        ...
        let tatBadge = "", isBefore6PM = orderDateObj.getHours() < 18;
        ...
        let mergeBadge = "", orderDateStr = parseCustomDate(order.date).toDateString();
        ...
        grid.innerHTML += `
            <div ${clickAction} class="${vipClass} rounded-2xl p-4 shadow-lg flex flex-col justify-between relative overflow-hidden group hover-card ${cursorStyle} border">
                ${lockOverlay}
                ...
                <div class="mb-4 relative z-10">
                    <div class="flex justify-between items-start mb-1.5"><h3 class="font-black text-sm text-white tracking-wider">${order.orderId}</h3><div class="flex flex-col gap-1 items-end"><div class="flex gap-1">${vipBadge}${splitBadge}</div></div></div>
                    <p class="text-indigo-300 font-bold text-xs leading-tight text-wrap-custom mb-3">${order.shopName}</p>
                    <div class="flex gap-1 items-center flex-wrap mb-3"><span class="bg-[#0B1121] text-[9px] px-2 py-1 rounded text-slate-400 font-mono tracking-widest border border-slate-800">${exactDateStr}</span><span class="bg-[#0B1121] ${timeData.color} text-[9px] px-2 py-1 rounded font-black tracking-widest border border-slate-800">⏱️ ${timeData.text}</span>${tatBadge}${slaBadge}${mergeBadge}</div>
                    <div class="mt-3 flex flex-wrap gap-2 items-center"><span class="text-[9px] font-black px-2 py-1 rounded tracking-widest uppercase border ${isCod ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}">${order.paymentMode || 'N/A'}</span><span class="text-[10px] font-black text-yellow-400 bg-yellow-900/20 px-2 py-1 rounded border border-yellow-500/20">₹${order.totalValue || '0'}</span><span class="text-[10px] font-black text-slate-300 bg-slate-800 px-2 py-1 rounded border border-slate-700">Qty: ${orderItemsCount}</span></div>
                </div>
                <div>
                    <div class="bg-[#0B1121] p-2.5 rounded-lg mb-3 border border-slate-800 flex justify-between items-center group-hover:border-indigo-500/30 transition-colors"><span class="text-[9px] text-slate-500 uppercase font-black tracking-widest">Next Action:</span><span class="text-[10px] text-emerald-400 font-black bg-emerald-900/20 px-2 py-1 rounded truncate max-w-[55%] border border-emerald-500/10">${nextStepName}</span></div>
                    <div class="flex justify-between text-[9px] text-slate-500 mb-1.5 font-black uppercase tracking-widest"><span>Pipeline Progress</span><span class="text-indigo-400">${order.completedStages}/9</span></div>
                    <div class="w-full bg-[#0B1121] rounded-full h-1.5 overflow-hidden border border-slate-800"><div class="bg-gradient-to-r from-indigo-600 to-purple-500 h-1.5 rounded-full relative" style="width: ${progress}%"><div class="absolute inset-0 bg-white/20 w-full animate-[pulse_2s_ease-in-out_infinite]"></div></div></div>
                </div>
            </div>
        `;
    });
...
async function executeMerge(primaryOrderId, shopName, secondaryIds, targetBtn) {
    ...
    try {
        const res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'mergeOrders', primaryOrderId: primaryOrderId, secondaryIds: secondaryIds, shopName: shopName }) });
        const data = await res.json();
        if (data.status === 'success') { showNotification("MERGE COMPLETE", "Orders have been physically combined in the database."); closeModal(); fetchOrders(true); } 
        else { alert(data.message); targetBtn.innerText = "Merge Failed"; }
    } catch(e) { alert("Network error during merge execution."); targetBtn.innerText = "Retry Merge"; targetBtn.disabled = false; }
}
...
function openModal(orderId) {
    ...
    if (order.isVIP) document.getElementById('modalVipBadge').classList.remove('hidden'); else document.getElementById('modalVipBadge').classList.add('hidden');
    ...
    let tatBadgeEl = document.getElementById('modalTatTimer'), isBefore6PM = orderDateObj.getHours() < 18;
    if (isBefore6PM && order.completedStages < 5) { 
        let targetTime = orderDateObj.getTime() + (60 * 60 * 1000); 
    ...
    } else tatBadgeEl.className = "hidden";

    let isCod = order.paymentMode && order.paymentMode.toUpperCase().includes('COD'); 
    let payBadge = document.getElementById('modalPaymentMode'); payBadge.innerText = order.paymentMode || 'N/A';
    if (isCod) payBadge.className = `text-[10px] px-2 py-1 rounded font-black tracking-widest uppercase border bg-orange-500/10 text-orange-400 border-orange-500/20`; else payBadge.className = `text-[10px] px-2 py-1 rounded font-black tracking-widest uppercase border bg-emerald-500/10 text-emerald-400 border-emerald-500/20`;
    ...
    let itemsHtml = `<div class="mb-6 bg-[#131C31] border border-slate-700 rounded-xl p-4 shadow-inner"><h3 class="text-xs font-black text-white uppercase tracking-widest mb-3 flex items-center gap-2"><span class="text-indigo-400 text-lg">📦</span> Order Manifest</h3><div class="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 hide-scrollbar">`;
    if (order.items && order.items.length > 0) {
        order.items.forEach(item => {
            ...
            itemsHtml += `<div class="flex items-start gap-3 bg-[#0B1121] p-2.5 rounded-lg border border-slate-800">${imgTag}<div class="flex-1 min-w-0"><p class="text-xs text-slate-300 font-bold leading-tight text-wrap-custom">${item.name}</p><div class="mt-2 flex items-center gap-2"><span class="text-[9px] font-black text-slate-500 uppercase tracking-widest">Qty:</span><span class="text-xs font-black text-indigo-400 bg-indigo-900/30 border border-indigo-500/20 px-2 py-0.5 rounded shadow-inner">${item.qty}</span></div></div></div>`;
        });
    } else itemsHtml += `<p class="text-sm text-slate-600 italic px-2">No SKU data found for this order.</p>`; 
    itemsHtml += `</div></div>`; container.innerHTML += itemsHtml;

    let mergeActionHtml = "", orderDateStr = parseCustomDate(order.date).toDateString();
    ...
    let needsShare = false, lastCompletedStage = order.completedStages;
    const stagesRequiringShare = [1, 3, 4, 5, 8]; 
    if (stagesRequiringShare.includes(lastCompletedStage)) { if (localStorage.getItem('shared_' + order.orderId + '_' + lastCompletedStage) !== 'true') needsShare = true; }
    
    for (let i = 0; i < 9; i++) { 
        const stageNum = i + 1, isCompleted = i < order.completedStages, isActive = i === order.completedStages, isLocked = i > order.completedStages;
        ...
        let uiHtml = '';
        if (isCompleted) {
            let cellData = order.stageUrls ? order.stageUrls[i] : "", timestampDisplay = "", timeTakenDisplay = "", thisEpoch = null, previousEpoch = parseCustomDate(order.date).getTime(); 
            let staffName = "System", staffMatch = cellData ? cellData.match(/STAFF:\s*(.+)/) : null;
            if (staffMatch) { staffName = staffMatch[1].trim(); cellData = cellData.replace(/\|\|\s*STAFF:\s*.+/, '').trim(); }
            if (i > 0 && order.stageUrls[i-1]) { let prevMatch = order.stageUrls[i-1].match(/TIME:\s*(\d+)/); if (prevMatch) previousEpoch = parseInt(prevMatch[1]); }
            if (cellData && cellData.includes("TIME:")) {
                let match = cellData.match(/TIME:\s*(\d+)/);
                if (match) {
                    thisEpoch = parseInt(match[1]); let d = new Date(thisEpoch); 
                    let day = String(d.getDate()).padStart(2, '0'), month = String(d.getMonth() + 1).padStart(2, '0'), year = String(d.getFullYear()).slice(-2), timeStr = d.toLocaleTimeString('en-US', {hour: '2-digit', minute:'2-digit'});
                    ...
                    let diffMins = Math.max(0, Math.round((thisEpoch - previousEpoch) / 60000)), timeFormatted = diffMins < 60 ? `${diffMins} Mins` : `${Math.floor(diffMins / 60)} Hour ${diffMins % 60} Mins`;
                    timeTakenDisplay = `<span class="text-[10px] text-orange-300 font-bold bg-orange-950/50 px-2 py-0.5 rounded border border-orange-800/50 ml-2 shadow-sm">⏱️ Took: ${timeFormatted}</span><span class="text-[10px] text-blue-200 font-black bg-blue-900/60 px-2 py-0.5 rounded border border-blue-400/50 ml-2 shadow-sm">👤 By: ${staffName}</span>`;
            ...
            if (cellData && cellData.includes("||")) {
                ...
                if (parts[0].includes("http")) urlData = parts[0].trim(); else { actionStr = parts[0].trim().replace(/^\[|\]$/g, ''); if (parts.length > 1 && parts[1].includes("http")) urlData = parts[1].trim(); }
            } else { if (cellData && !cellData.includes("http")) actionStr = cellData.trim().replace(/^\[|\]$/g, ''); else urlData = cellData ? cellData.trim() : ""; }

            let fileUrls = urlData ? urlData.split(',') : [], hasFiles = false, previewHtml = `<div class="flex flex-col gap-2 mt-3">`;
            if (actionStr && actionStr !== "Completed") previewHtml += `<div class="bg-indigo-900/30 border border-indigo-500/30 p-2.5 rounded-lg mb-2 text-indigo-300 text-xs font-bold leading-relaxed whitespace-pre-wrap shadow-inner">📋 ${actionStr}</div>`;
            
            fileUrls.forEach((url, index) => {
                url = url.trim();
                if (url.includes("http")) {
                    hasFiles = true;
                    if (stageNum === 9 || stageNum === 7 || url.toLowerCase().includes("audio")) previewHtml += `<audio controls src="${url}" class="h-10 w-full outline-none bg-slate-800 rounded border border-slate-700 mt-2"></audio>`; 
                    else { 
                        let imgThumbnailUrl = url, driveMatch = url.match(/\/d\/(.*?)\//);
...
function handleFileSelection(inputElement, stageNum) {
    const newFiles = Array.from(inputElement.files), validFiles = [];
    for (let f of newFiles) { if (f.size > 6 * 1024 * 1024) alert(`File "${f.name}" is too large (${(f.size/1024/1024).toFixed(1)}MB). Please keep audio recordings under 6MB.`); else validFiles.push(f); }
    if (validFiles.length === 0) { inputElement.value = ''; return; }
    queuedFiles = queuedFiles.concat(validFiles);
    const statusLabel = document.getElementById(`fileStatusText_${stageNum}`), clearBtn = document.getElementById(`clearFilesBtn_${stageNum}`), container = document.getElementById(`fileStatusContainer_${stageNum}`), previewBox = document.getElementById(`previewBox_${stageNum}`);
    if (queuedFiles.length > 0) {
        statusLabel.innerText = `✓ ${queuedFiles.length} File(s) Ready`; statusLabel.classList.remove('hidden'); 
        if (clearBtn) clearBtn.classList.remove('hidden'); if (container) container.classList.remove('hidden');
        if (previewBox) {
            previewBox.innerHTML = ''; previewBox.classList.remove('hidden');
            queuedFiles.forEach(file => {
                if (file.type.startsWith('audio/')) previewBox.innerHTML += `<div class="w-12 h-12 bg-indigo-900/50 flex flex-col items-center justify-center rounded border border-indigo-500 text-[8px] text-indigo-300 font-bold p-1 overflow-hidden shadow-inner">🎤<br/>Audio</div>`; 
                else { const reader = new FileReader(); reader.onload = function(e) { previewBox.innerHTML += `<img src="${e.target.result}" class="w-12 h-12 object-cover rounded border border-slate-600 shadow-sm">`; }; reader.readAsDataURL(file); }
            });
        }
    }
}
...
function clearQueuedFiles(stageNum) {
    ...
    if (document.getElementById(`previewBox_${stageNum}`)) { document.getElementById(`previewBox_${stageNum}`).innerHTML = ''; document.getElementById(`previewBox_${stageNum}`).classList.add('hidden'); }
}

window.shareToGroup = function(stageNum, fileUrlsString) {
    const orderId = currentActiveOrder.orderId, shop = currentActiveOrder.shopName, stageName = STAGE_NAMES[stageNum - 1]; 
...
function generateEODReport() {
    if (!window.appData || !window.appData.rawArray || window.appData.rawArray.length === 0) { alert("No data available to generate report."); return; }
    let today = new Date(); today.setHours(0,0,0,0);
    let todayOrders = window.appData.rawArray.filter(o => { let d = parseCustomDate(o.date); d.setHours(0,0,0,0); return d.getTime() === today.getTime(); });
    let totalReceived = todayOrders.length, totalValue = todayOrders.reduce((sum, o) => sum + (o.totalValue || 0), 0);
    let totalCompleted = todayOrders.filter(o => o.isFullyCompleted).length, totalDispatched = todayOrders.filter(o => o.completedStages >= 5 && !o.isFullyCompleted).length, pending = totalReceived - totalCompleted - totalDispatched;
    let reportDate = new Date().toLocaleDateString('en-GB'), staffName = currentUserName ? currentUserName.toUpperCase() : 'ADMIN';
    let msg = `*📊 DAILY EOD REPORT - YASH MARKETING*\n\n*📅 Date:* ${reportDate}\n*🧑‍💼 Staff:* ${staffName}\n\n*📦 Orders Received Today:* ${totalReceived}\n*💰 Total Value:* ₹${totalValue.toLocaleString()}\n*✅ Orders Delivered/Final:* ${totalCompleted}\n*🚚 Orders Dispatched:* ${totalDispatched}\n*⏳ Pending Orders:* ${pending}\n\n_System Generated via MDO OS_`;
    window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(msg)}`, '_blank');
}
...
function generateTallyCSV(orderId) {
    ...
    order.items.forEach(item => { csvContent += `${formattedDate},Sales Order,${order.orderId},"${order.shopName}","${item.name}",${item.qty},0,0\n`; });
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }), link = document.createElement("a");
    link.href = URL.createObjectURL(blob); link.download = `Tally_SO_${order.orderId}.csv`; link.style.display = 'none';
...
function compressImage(file) {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            const canvas = document.createElement('canvas'), MAX_WIDTH = 800;
...
async function submitStage(stageNum) {
    const btn = document.getElementById(`submitBtn_${stageNum}`), statusLabel = document.getElementById(`status_${stageNum}`);
    ...
    if (stageNum === 6) {
        let runner = document.getElementById('runnerSelect').value; if (runner === "Others") runner = document.getElementById('runnerOther').value;
        let transport = document.getElementById('transportSelect').value; if (transport === "Others") transport = document.getElementById('transportOther').value;
        if (!runner || !transport) { alert("Accountability requires data! Select both a Delivery Person and a Transport partner."); if (btn) { btn.disabled = false; btn.innerText = "Allocate Logistics & Proceed"; } return; }
        payload.actionText = `Runner: ${runner} | Transport: ${transport}`;
    }
    
    let fileRequired = true;
    if (payload.isNoResponse || stageNum === 9 || stageNum === 6 || stageNum === 2 || (stageNum === 7 && !isCod)) fileRequired = false; 
    if (fileRequired && queuedFiles.length === 0) {
        if (stageNum === 7 && isCod) alert("For COD Orders, Call Recording is MANDATORY."); else alert("A file proof (Screenshot/Photo) is mandatory."); 
        return; 
    }
    ...
    for (let i = 0; i < queuedFiles.length; i++) {
        let f = queuedFiles[i];
        if (f.type.startsWith('image/')) payload.files.push(await compressImage(f)); else payload.files.push(await getBase64(f)); 
    }
    ...
    if (stageNum === 3) {
        ...
        currentActiveOrder.items.forEach((item, idx) => {
            ...
            if (chk && qtyInput) {
                let isChecked = chk.checked, userQty = parseInt(qtyInput.value) || 0, origQty = parseInt(item.qty);
                if (isChecked && userQty > 0) { processed.push({ name: item.name, qty: userQty }); if (userQty < origQty) short.push({ name: item.name, qty: origQty - userQty }); } else short.push({ name: item.name, qty: origQty }); 
            }
        });
    ...
    try {
        ...
        if (statusLabel) { statusLabel.innerText = "❌ " + (err.message || "Sync Failed. Network Drop."); statusLabel.className = "text-[10px] font-black mt-3 text-pink-500 block text-center uppercase"; }
        if (btn) { btn.innerText = "Retry Transmission (Tap Again)"; btn.classList.remove('bg-indigo-600', 'hover:bg-indigo-500'); btn.classList.add('bg-pink-600', 'hover:bg-pink-500'); btn.disabled = false; }
    }
}
