                ...
                <div class="space-y-3">
                    ${sortedSKUs.length > 0 ? sortedSKUs.map((sku, i) => `
                        <div class="bg-[#0B1121] p-2.5 rounded-lg border border-slate-800">
                            <div class="text-xs font-bold text-slate-300 truncate mb-1">${i+1}. ${sku[0]}</div>
                            <div class="flex justify-between items-end">
                                <span class="text-sm font-black text-yellow-400">${sku[1].qty} <span class="text-[9px] text-slate-500">QTY</span></span>
                                <span class="text-xs font-black text-emerald-400">₹${sku[1].value.toLocaleString()}</span>
                            </div>
                        </div>
                    ...
                    ${sortedShops.length > 0 ? sortedShops.slice(0, 5).map((v, i) => `
                        <div class="relative w-full bg-[#0B1121] rounded-lg p-2.5 overflow-hidden border border-slate-800 group">
                            ...
                            <div class="relative z-10">
                                <div class="text-xs font-bold text-slate-200 truncate group-hover:text-white">${v[0]}</div>
                                <div class="text-[9px] text-slate-400 uppercase tracking-widest mb-1">📍 ${v[1].area}</div>
                                <div class="text-sm font-black text-emerald-400">₹${v[1].value.toLocaleString()}</div>
                            </div>
                        </div>
            ...
            <div class="glass-panel p-5 rounded-2xl lg:col-span-1">
                ...
                <div class="space-y-3">
                    ${sortedCities.length > 0 ? sortedCities.map((city, i) => `
                        <div class="bg-[#0B1121] p-2.5 rounded-lg border border-slate-800">
                            <div class="text-xs font-bold text-slate-300 truncate mb-1">#${i+1} ${city[0]}</div>
                            <div class="flex justify-between items-end">
                                <span class="text-sm font-black text-blue-400">${city[1].orders} <span class="text-[9px] text-slate-500">ORD</span></span>
                                <span class="text-xs font-black text-emerald-400">₹${city[1].value.toLocaleString()}</span>
                            </div>
                        </div>
...
function renderPipeline() {
    ...
    let filterHtml = `
        <div onclick="setStageFilter(null)" class="snap-start cursor-pointer ${activeStageFilter === null ? 'bg-indigo-600 border-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.4)]' : 'bg-[#131C31] border-slate-800 hover:bg-slate-800'} flex flex-col justify-between p-3 rounded-xl border min-w-[120px] shrink-0 transition-all">
    ...
    STAGE_NAMES.forEach((name, idx) => {
        ...
        filterHtml += `
            <div onclick="setStageFilter(${idx})" class="snap-start cursor-pointer ${isActive ? 'bg-indigo-600 border-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.4)]' : 'bg-[#131C31] border-slate-800 hover:bg-slate-800'} flex flex-col justify-between p-3 rounded-xl border min-w-[130px] shrink-0 transition-all">
    ...
    displayOrders.forEach(order => {
        ...
        if (order.creditLocked && !order.ceoOverride) {
            let unlockBtn = isOwner ? `
                <button onclick="event.stopPropagation(); executeUnblock('${order.orderId}')" class="mt-3 w-full bg-red-600 hover:bg-red-500 text-white text-[10px] py-1.5 rounded font-black uppercase tracking-widest transition-all">🔓 CEO Override: Unblock</button>
        ...
        let vipBadge = "";
        if (order.isVIP) {
            vipBadge = `<span class="bg-red-500 text-white text-[9px] px-2 py-1 rounded font-black uppercase shadow-[0_0_10px_red] animate-pulse shrink-0">🔥 VIP</span>`;
        }
        
        let slaBadge = "";
        if (timeData.isSLAWarning) {
            slaBadge = `<span class="text-[9px] bg-yellow-500/20 text-yellow-400 font-black border border-yellow-500 px-2 py-1 rounded animate-pulse tracking-widest shrink-0">⏳ NEAR BREACH</span>`;
        }
...
function openModal(orderId) {
    ...
    if (order.completedStages < 3 && secondaryOrders.length > 0 && !order.creditLocked && !isAlreadyMerged) {
        ...
        mergeActionHtml = `
            <button onclick="executeMerge('${order.orderId}', '${order.shopName}', '${secondaryIds}', this)" class="w-full mt-2 mb-4 bg-blue-600/20 hover:bg-blue-600 text-blue-400 hover:text-white font-black py-3 rounded-xl shadow-lg transition-all text-xs flex items-center justify-center gap-2 uppercase tracking-widest border border-blue-500/50">
    ...
    for (let i = 0; i < 9; i++) { 
        ...
        if (isCompleted) {
                    ...
                    if (stageNum === 9 || stageNum === 7 || url.toLowerCase().includes("audio")) {
                        ...
                        previewHtml += `
                            <div class="mt-2 bg-[#0B1121] rounded-lg border border-slate-700 overflow-hidden relative group shadow-sm">
                                <p class="text-[10px] font-bold text-indigo-400 p-2 border-b border-slate-700 bg-slate-800/50">🖼️ Uploaded Proof ${index + 1}</p>
                                <a href="${url}" target="_blank" class="block relative">
                                    ...
                                    <div class="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                        <span class="text-white text-xs font-black bg-indigo-600 px-3 py-1.5 rounded-full shadow-lg">🔍 Click to Enlarge</span>
                                    </div>
                                </a>
                            </div>
                        `; 
            ...
            if (hasFiles && stagesRequiringShare.includes(stageNum)) {
                ...
                previewHtml += `
                    <button onclick="shareToGroup(${stageNum}, '${urlData}')" class="mt-2 w-full ${btnClass} text-[10px] font-black py-3 rounded-lg shadow transition-colors flex items-center justify-center gap-2 uppercase tracking-widest border">
            ...
            uiHtml = `
                <div class="bg-emerald-900/10 border border-emerald-500/20 rounded-xl p-4 hover:bg-emerald-900/20 transition">
                    <div class="flex justify-between items-start">
                        <div>
                            <span class="text-emerald-400 text-sm font-black block mb-1.5">${stageNum}. ${displayStageName}</span>
                            <div class="flex items-center gap-2 flex-wrap">${timestampDisplay}${timeTakenDisplay}</div>
                        </div>
                        <span class="bg-emerald-950 text-emerald-500 font-black text-[9px] px-2 py-1 rounded shadow-inner uppercase tracking-widest shrink-0 ml-2">✔ DONE</span>
                    </div>
                    ${previewHtml}
                </div>
            `;
            ...
            uiHtml = `
                <div class="bg-slate-800/20 border border-slate-800 rounded-xl p-4 flex justify-between items-center grayscale opacity-50">
                    ...
                    <span class="text-slate-600 text-[9px] font-mono tracking-widest bg-slate-900 px-2 py-1 rounded">🔒 LOCKED</span>
                </div>
            `;
        }
        else if (isActive) {
            ...
            if (stageNum === 1) { 
                inputHtml = `
                    <button onclick="generateTallyCSV('${order.orderId}')" class="w-full mt-4 mb-2 bg-[#EAB308] hover:bg-[#CA8A04] text-black font-black py-3.5 rounded-xl shadow-[0_0_20px_rgba(234,179,8,0.3)] transition-all active:scale-95 text-sm flex items-center justify-center gap-2">
                        <span>📊</span> Export Sales Order for Tally (CSV)
                    </button>
                    <p class="text-[10px] font-black text-slate-400 mt-4 mb-1.5 uppercase tracking-widest flex items-center gap-2"><span class="text-indigo-400">⚡</span> Upload Balance Check Proof (Required)</p>
                    ...
                    <div class="flex gap-2 w-full">
                        <label class="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white text-center py-3 rounded-xl cursor-pointer text-xs font-bold transition shadow-inner">
                            📸 Live Camera
                            <input type="file" accept="image/*" capture="environment" class="hidden" onchange="handleFileSelection(this, ${stageNum})">
                        </label>
                        <label class="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white text-center py-3 rounded-xl cursor-pointer text-xs font-bold transition shadow-inner">
                            🖼️ Multi Gallery
                            <input type="file" accept="image/*" multiple class="hidden" onchange="handleFileSelection(this, ${stageNum})">
                        </label>
                    </div>
                    <div id="previewBox_${stageNum}" class="flex flex-wrap gap-2 mt-3 hidden bg-[#0B1121] p-2 rounded-lg border border-slate-700"></div>
                    <div class="flex items-center justify-between mt-2 bg-emerald-900/10 rounded px-2 border border-emerald-500/10 hidden" id="fileStatusContainer_${stageNum}">
                        <p id="fileStatusText_${stageNum}" class="text-[10px] font-black text-emerald-400 py-2 hidden"></p>
                        <button id="clearFilesBtn_${stageNum}" onclick="clearQueuedFiles(${stageNum})" class="text-[10px] font-bold text-pink-400 hidden px-2 py-1 bg-pink-900/30 rounded border border-pink-500/30">Clear Files</button>
                    </div>
                    <button id="submitBtn_${stageNum}" onclick="submitStage(${stageNum})" class="w-full mt-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black py-3.5 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all active:scale-95 text-lg">Confirm Check & Execute</button>
                `;
            }
            else if (stageNum === 6) { 
                inputHtml += `
                    <div class="bg-[#0B1121] border border-slate-700 p-4 rounded-xl mt-4 mb-4 shadow-inner">
                        <p class="text-[10px] text-slate-400 mb-3 uppercase font-black tracking-widest flex items-center gap-2"><span class="text-indigo-400 text-base">🚚</span> Logistics Tracking Engine</p>
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">1. Who took the order?</label>
                        ...
                        <input type="text" id="runnerOther" placeholder="Enter person's name..." class="hidden w-full bg-[#131C31] border border-indigo-500/50 rounded-lg p-2.5 text-white outline-none mb-4 font-bold text-sm">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">2. Which Transport?</label>
                        ...
                        <input type="text" id="transportOther" placeholder="Enter transport name..." class="hidden w-full bg-[#131C31] border border-indigo-500/50 rounded-lg p-2.5 text-white outline-none mb-4 font-bold text-sm">
                        <button onclick="submitStage(${stageNum})" id="submitBtn_${stageNum}" class="w-full mt-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black py-3.5 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all active:scale-95 text-lg">Allocate Logistics & Proceed</button>
                    </div>
                `;
            }
                ...
                inputHtml += `
                    <div class="mt-4 mb-4">
                        <p class="text-[10px] text-slate-400 mb-2 uppercase tracking-widest">Step 1: Communication</p>
                        <a href="${waUrl}" target="_blank" class="w-full bg-[#25D366] hover:bg-[#128C7E] text-white font-black py-3 rounded-xl flex justify-center items-center gap-2 shadow-lg transition-all active:scale-95 mb-3">Send WhatsApp Message</a>
                        <label class="flex items-center gap-3 cursor-pointer bg-pink-500/10 border border-pink-500/30 p-3 rounded-xl hover:bg-pink-500/20 transition-colors">
                            <input type="checkbox" id="noResponse_${stageNum}" class="w-5 h-5 rounded border-slate-600 bg-slate-800 text-pink-500 cursor-pointer shrink-0">
                            <span class="text-[11px] text-pink-400 font-black uppercase tracking-widest text-wrap-custom">Dealer Didn't Respond / Ignore</span>
                        </label>
                    </div>
                `;
                ...
                if (stageNum === 7 && isCod) { 
                    inputHtml += `
                        <div class="bg-indigo-900/20 p-3 rounded-xl border border-indigo-500/30 mb-4">
                            <p class="text-[10px] font-black text-indigo-400 uppercase tracking-widest flex items-center gap-2 mb-2"><span class="text-indigo-400">🎤</span> Upload Call Recording (Mandatory for COD)</p>
                            <input type="file" accept="audio/*" class="w-full text-xs text-slate-300 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:font-black file:bg-indigo-600 file:text-white bg-[#0B1121] p-1.5 rounded-xl border border-slate-700 cursor-pointer" onchange="handleFileSelection(this, ${stageNum})">
                            <div id="previewBox_${stageNum}" class="flex flex-wrap gap-2 mt-3 hidden bg-[#0B1121] p-2 rounded-lg border border-slate-700"></div>
                            <div class="flex items-center justify-between mt-2 bg-emerald-900/10 rounded px-2 border border-emerald-500/10 hidden" id="fileStatusContainer_${stageNum}">
                                <p id="fileStatusText_${stageNum}" class="text-[10px] font-black text-emerald-400 py-2 hidden"></p>
                                <button id="clearFilesBtn_${stageNum}" onclick="clearQueuedFiles(${stageNum})" class="text-[10px] font-bold text-pink-400 hidden px-2 py-1 bg-pink-900/30 rounded border border-pink-500/30">Clear</button>
                            </div>
                        </div>
                    `;
                }
                ...
                inputHtml += `
                    <p class="text-[10px] text-slate-400 mb-2 uppercase tracking-widest">Step 2: Submit</p>
                    <button id="submitBtn_${stageNum}" onclick="submitStage(${stageNum})" class="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-black py-3.5 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all active:scale-95 text-lg">Mark as Completed</button>
                `;
            }
            else {
                if (stageNum === 3) { 
                    inputHtml += `
                        <div class="bg-[#0B1121] border border-slate-700 p-4 rounded-xl mt-4 mb-4 shadow-inner">
                            <p class="text-sm text-slate-300 mb-4 uppercase font-black tracking-widest flex items-center gap-2"><span class="text-indigo-400 text-xl">☑</span> Confirm Available Stock</p>
                        ...
                        inputHtml += `
                            <div class="flex flex-col bg-[#1E293B] p-4 rounded-xl border-2 border-slate-700 hover:border-indigo-500 transition-all shadow-md">
                                <div class="flex items-start gap-4">
                                    <input type="checkbox" id="chk_item_${idx}" checked class="w-6 h-6 rounded border-slate-600 bg-slate-800 text-indigo-500 mt-1 cursor-pointer shrink-0">
                                    ${imgTag}
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="mt-4 flex items-center justify-between bg-[#0B1121] p-2 rounded-lg border border-slate-800">
                                            ...
                                            <input type="number" id="qty_item_${idx}" value="${item.qty}" max="${item.qty}" min="0" class="w-16 bg-[#131C31] text-indigo-400 text-lg font-black p-1.5 rounded-lg border border-indigo-500/30 text-center outline-none focus:border-indigo-400">
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    inputHtml += `
                            ...
                            <p class="text-[10px] text-orange-400 mt-4 font-bold leading-tight bg-orange-900/10 p-3 rounded-lg border border-orange-500/20 text-center uppercase tracking-widest">* Unchecked items or reduced quantities automatically create a new SPLIT-ORDER.</p>
                        </div>
                    `;
                }
                ...
                if (stageNum === 4) { 
                    inputHtml += `
                        <button onclick="generateShippingLabel('${order.orderId}')" class="w-full mt-4 mb-2 bg-blue-600 hover:bg-blue-500 text-white font-black py-3 rounded-xl shadow-[0_0_20px_rgba(37,99,235,0.3)] transition-all active:scale-95 text-sm flex items-center justify-center gap-2">🖨️ Print Dynamic A6 Shipping Label</button>
                    `;
                }

                if (stageNum === 9) { 
                    inputHtml += `
                        <p class="text-[10px] font-black text-slate-400 mt-4 mb-2 uppercase tracking-widest flex items-center gap-2"><span class="text-indigo-400">🎤</span> Record Audio / Call (Optional)</p>
                        <input type="file" accept="audio/*" class="w-full text-xs text-slate-300 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:font-black file:bg-indigo-600 file:text-white bg-[#0B1121] p-1.5 rounded-xl border border-slate-700 cursor-pointer" onchange="handleFileSelection(this, ${stageNum})">
                    `;
                } else {
                    inputHtml += `
                        <p class="text-[10px] font-black text-slate-400 mt-4 mb-1.5 uppercase tracking-widest flex items-center gap-2"><span class="text-indigo-400">⚡</span> Upload Proof (Required)</p>
                        <p class="text-[9px] text-orange-400 mb-3 italic tracking-widest">Tip: Use "Multi Gallery" if app freezes on Live Camera.</p>
                        <div class="flex gap-2 w-full">
                            <label class="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white text-center py-3 rounded-xl cursor-pointer text-xs font-bold transition shadow-inner">📸 Live Camera<input type="file" accept="image/*" capture="environment" class="hidden" onchange="handleFileSelection(this, ${stageNum})"></label>
                            <label class="flex-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white text-center py-3 rounded-xl cursor-pointer text-xs font-bold transition shadow-inner">🖼️ Multi Gallery<input type="file" accept="image/*" multiple class="hidden" onchange="handleFileSelection(this, ${stageNum})"></label>
                        </div>
                    `;
                }
                ...
                if (stageNum === 9) { 
                    inputHtml += `
                        <div class="mt-4">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Customer Rating</label>
                            <input type="number" id="ratingInput" min="1" max="5" placeholder="Enter Score (1-5)" class="w-full bg-[#0B1121] border border-slate-700 rounded-xl p-3 text-white outline-none font-bold text-lg text-center shadow-inner">
                        </div>
                    `;
                }
                
                inputHtml += `
                    <button id="submitBtn_${stageNum}" onclick="submitStage(${stageNum})" class="w-full mt-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black py-3.5 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all active:scale-95 text-lg">Execute & Proceed</button>
                `;
            }
            ...
            if (needsShare) {
                uiHtml = `
                    <div class="bg-orange-900/10 border-2 border-orange-500/30 rounded-xl p-5 relative overflow-hidden mt-4">
                        ...
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-white font-black text-lg text-opacity-50">${stageNum}. ${displayStageName}</span>
                            <span class="bg-orange-500 text-white text-[9px] font-black px-2 py-1 rounded tracking-widest uppercase">🔒 LOCKED</span>
                        </div>
                        <p class="text-xs text-orange-400 font-bold mt-3">⚠️ Action Required: Upar scroll karein aur agla step unlock karne ke liye flashing green "Share to Group" button daba kar update share karein.</p>
                    </div>
                `;
            } else {
                uiHtml = `
                    <div class="bg-[#131C31] border-2 border-indigo-500 rounded-xl p-5 shadow-[0_0_30px_rgba(79,70,229,0.15)] relative overflow-hidden transform scale-[1.02]">
                        ...
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-white font-black text-lg">${stageNum}. ${displayStageName}</span>
                            <span class="bg-indigo-500 text-white text-[9px] font-black px-2 py-1 rounded animate-pulse tracking-widest uppercase">ACTION REQUIRED</span>
                        </div>
...
async function submitStage(stageNum) {
    ...
    if (stageNum === 6) { 
        ...
        if (runner === "Others") runner = document.getElementById('runnerOther').value;
        
        let transport = document.getElementById('transportSelect').value;
        if (transport === "Others") transport = document.getElementById('transportOther').value;
    ...
    if (stageNum === 9) { 
        ...
        if (!rating) { 
            ...
            if (btn) { btn.disabled = false; btn.innerText = "Execute & Proceed"; } 
            return; 
        }
